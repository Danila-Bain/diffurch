...
> {
    ...
    pub callback: Box<dyn Fn(State<N, S>) -> Output>,
    ...
    pub stream: [Box<dyn Fn(Output)>; SN],
    ...
    /// bool.
    pub filter: [Box<dyn Fn<State<N, S>, Output = bool>>; FN],
...
impl<const N: usize, const S: usize> Event<N,S> {
...
// /// ```
// pub fn new<Args: Tuple, Output, C: Fn<Args, Output = Output>(callback: C) -> Event<Args, (), Output> {
//     Event {
//         callback,
//         stream: [],
//         filter: [],

...
#[test]
fn constant() {
    let eq = Equation::constant(|| [1., -1., 0.]);
    let ic = [0., 0., 0.];
...
#[test]
fn ode2_lin() {
    let eq = Equation::ode2(|t, [x]| [2. * x / t]);
    let ic = |t: f64| [t * t];
    let sol = |t: f64| t * t;
...
#[test]
fn bouncing_ball() {
    let eq = Equation::ode(|[_x, dx]| [dx, -2.]).with_delay(f64::INFINITY);
    ...
    Solver::rk(&rk::RK98)
        ...
        .on_step(Event::ode2(|t, [x, _]| assert!((x - sol(t)).abs() < 1e-13)))
        .run(eq, ic, range);
}
